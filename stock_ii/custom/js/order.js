...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax': 'php_action/fetchOrder.php?shop=cosmetics',
			"responsive": true,
		'order': [],
		"columnDefs": [
	...
	if(orderId) {
		...
		$.ajax({
			...
			success:function(response) {				
				...
				// due 
				$("#due").val(response.order[4]);				

				// pay amount 
				$("#payAmount").val(response.order[4]);

				var paidAmount = response.order[5] 
				var dueAmount = response.order[4];							
				var grandTotal = response.order[4];
