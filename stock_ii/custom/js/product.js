...
$(document).ready(function() {
	...
	manageProductTable = $('#manageProductTable').DataTable({
		'ajax': 'php_action/fetchProduct.php?shop=cosmetics',
		"responsive": true,
		'order': [],
		"columnDefs": [
	...
	$("#addProductModalBtn").unbind('click').bind('click', function() {
		...
		$("#submitProductForm").unbind('submit').bind('submit', function() {
			...
			var productImage2 = $("#productImage2").val();
			var productImage3 = $("#productImage3").val();
			...
			// if(category == "") {
			// 	$("#category").after('<p class="text-danger">Brand category field is required</p>');
			...
			if(productImage  && productImage2 && productImage3 && productName && quantity && rate && brandName  && categoryName && productStatus) {
...
function editProduct(productId = null, dbname) {
	...
	if(productId) {
		...
		$.ajax({
			url: 'php_action/fetchSelectedProduct.php?shop=cosmetics',
			type: 'post',
			...
			// alert(response.product_image);
				...
				$("#getProductImage2").attr('src', 'stock/'+response.product_image1);

				$("#editProductImage2").fileinput({		      
				});  

				$("#getProductImage3").attr('src', 'stock/'+response.product_image2);

				$("#editProductImage3").fileinput({		      
				});  
		  ...
		  // 		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
				...
				// code
				$("#editProductCode").val(response.code);
				// quantity
				$("#editQuantity").val(response.quantity);
				// rate
				$("#editRate").val(response.rate);
				// brand name
				$("#editOffer").val(response.offer);
				// brand name
				$("#editDescription").val(response.description);
				// brand name
				$("#editSpec").val(response.specifications);
				// brand name
				...
				// category name
				$("#editCategoryName2").val(response.categories_id2);
				// category name
				$("#editCategoryName3").val(response.categories_id3);
				// status
				...
				$("#editProductForm").unbind('submit').bind('submit', function() {
					...
					var productImage2 = $("#editProductImage2").val();
					var productImage3 = $("#editProductImage3").val();
					...
					// if(category == "") {
					// 	$("#editCategory").after('<p class="text-danger">Brand category field is required</p>');
					// 	$('#editCategory').closest('.form-group').addClass('has-error');
					// } else {
					// 	// remov error text field
					// 	$("#editCategory").find('.text-danger').remove();
					// 	// success out for form 
					...
					if(productName && quantity && rate && brandName && category && categoryName && productStatus) {
						// submit loading button
					...
					if(productImage) {
						...
						$.ajax({
				          ...
				          // reload the manage student table
									...
									$.ajax({
										url: 'php_action/fetchProductImageUrl.php?shop=cosmetics&i='+productId,
										type: 'post',
										success:function(response) {
		...
		// update the product image2				
		$("#updateProductImageForm2").unbind('submit').bind('submit', function() {					
			// form validation
			var productImage2 = $("#editProductImage2").val();					
			
			if(productImage2 == "") {
				$("#editProductImage2").closest('.center-block').after('<p class="text-danger">Product Image field is required</p>');
				$('#editProductImage2').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#editProductImage2").find('.text-danger').remove();
				// success out for form 
				$("#editProductImage2").closest('.form-group').addClass('has-success');	  	
			}	// /else
				...
				$.ajax({
				  ...
				  // reload the manage student table
							...
							$.ajax({
								...
								type: 'post',
								success:function(response) {
								$("#getProductImage2").attr('src', response);		
								}
							});																		
		...
		// update the product image3				
		$("#updateProductImageForm3").unbind('submit').bind('submit', function() {					
			// form validation
			var productImage3 = $("#editProductImage3").val();					
			
			if(productImage3 == "") {
				$("#editProductImage3").closest('.center-block').after('<p class="text-danger">Product Image field is required</p>');
				$('#editProductImage3').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#editProductImage3").find('.text-danger').remove();
				// success out for form 
				$("#editProductImage3").closest('.form-group').addClass('has-success');	  	
			}	// /else
				...
				$.ajax({
				  ...
				  // reload the manage student table
							...
							$.ajax({
								...
								type: 'post',
								success:function(response) {
								$("#getProductImage3").attr('src', response);		
								}
							});																		
...
function removeProduct(productId = null) {
	if(productId) {
		...
		$("#removeProductBtn").unbind('click').bind('click', function() {
			...
			$.ajax({
				url: 'php_action/removeProduct.php?shop=cosmetics',
				type: 'post',
				data: {productId: productId},
				dataType: 'json',
				success:function(response) {
