...
$(document).ready(function() {
	...
	manageCategoriesTable = $('#manageCategoriesTable').DataTable({
		'ajax' : 'php_action/fetchCategories.php?shop=cosmetics',
		'order': []
	...
	$('#addCategoriesModalBtn').unbind('click').bind('click', function() {
		...
		$("#submitCategoriesForm").unbind('submit').bind('submit', function() {
			...
			if(categoriesName && category && categoriesStatus) {
				var form = $(this);
				// button loading
...
function editCategories(categoriesId = null) {
	if(categoriesId) {
		...
		$.ajax({
			url: 'php_action/fetchSelectedCategories.php?shop=cosmetics',
			type: 'post',
