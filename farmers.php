...
<table id="myTable" class="display responsive nowrap" style="width:90%">
        ...
        if(isset($_GET['i'])){
            if($_GET['i'] == ''){
                $sql1 = "SELECT * FROM users WHERE type='farmer' and account_id != '' ORDER BY joined_date ASC ";
                $result = $connect->query($sql1);
            }else{
$sql1 = "SELECT * FROM users WHERE user_id = $merchant and type='farmer' and account_id != '' ORDER BY joined_date ASC ";
$result = $connect->query($sql1);
            }
        }else{
            $sql1 = "SELECT * FROM users WHERE type='farmer' ORDER BY joined_date ASC ";
            $result = $connect->query($sql1);
        }
