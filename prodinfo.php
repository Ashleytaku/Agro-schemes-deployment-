...
if(isset($_POST['merchant'])){
    ...
    $sql = "INSERT INTO projects(name, region, fundtotal, image, fundavailable, description, no_of_farmers, datesaved, status)
           VALUES('$name', $merchant,  '$price', '$url1', '$price', '$description', '$farmers', '$date', 1)";

if($connect->query($sql) === TRUE){

  $sql3 = "SELECT * FROM projects ORDER BY id DESC LIMIT 1";
$result3 = $connect->query($sql3);
$row3 = $result3->fetch_assoc();
$id = $row3['id'];
  $valid['messages'] = "You have successfully added a project into the system.";
...
if(isset($_POST['edit'])){
  ...
  $sql = "UPDATE projects SET name = '$name', description = '$description', no_of_farmers = '$farmers', fundtotal = '$price',
   status = '$status' WHERE id = $merchant";

if($connect->query($sql) === TRUE){

$valid['messages'] = "You have successfully edited a product in the system.<a href='prodinfo.php?i=$merchant'>refresh</a>";
// header("refresh: 2;  url=prodinfo.php?i=$merchant");
...
<br>
  ...
  <br>
    ...
    <input type="text"  class="form-control" value="<?php echo $row['fundtotal'];?>" name="price"  placeholder="0.00">
  </div>
  <div class="col-6">
  <br>
    <label for="inputAddress2" class="form-label">No of farmers</label>
    <input type="text"  class="form-control" value="<?php echo $row['no_of_farmers'];?>" name="farmers"  placeholder="">
  </div>
  <div class="col-6">
  <br>
